CLASS zcl_as_cargas DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_as_cargas IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    TYPES: tt_product  TYPE TABLE OF ztas_product WITH DEFAULT KEY,
           tt_vendor   TYPE TABLE OF ztas_vendor WITH DEFAULT KEY,
           tt_company  TYPE TABLE OF ztas_company WITH DEFAULT KEY,
           tt_customer TYPE TABLE OF ztas_customer WITH DEFAULT KEY.
    ...
    DELETE FROM ztas_product.
    DELETE FROM ztas_vendor.
    DELETE FROM ztas_company.
    DELETE FROM ztas_customer.

    MODIFY ztas_product  FROM TABLE @lt_product.
    MODIFY ztas_vendor   FROM TABLE @lt_vendor.
    MODIFY ztas_company  FROM TABLE @lt_company.
    MODIFY ztas_customer FROM TABLE @lt_customer.
